...
def compute_quality_flags(summary: DatasetSummary, missing_df: pd.DataFrame) -> Dict[str, Any]:
    ...
    # === НОВЫЕ ЭВРИСТИКИ ДОБАВЛЕНЫ ЗДЕСЬ ===
    ...
    for col in summary.columns:
        # Проверяем только нечисловые колонки
    ...
    # Простейший «скор» качества (обновлён с учётом новых эвристик)
    score = 1.0
    ...
    if has_high_cardinality:
        score -= 0.05  # небольшой штраф за высокую кардинальность
